...
export async function sendJoinNotification({
  ...
  await resend.emails.send({
    ...
    to: recipientEmail,
    subject: `${joinerName} è entrato nel ricordo "${memoryTitle}"`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 24px; background: #fff; color: #111;">
        ...
        <h1 style="font-size: 22px; font-weight: 600; margin: 0 0 8px 0; line-height: 1.3;">
          ${joinerName} ha accettato il tuo invito
        </h1>

        <p style="font-size: 16px; color: #555; margin: 0 0 32px 0;">
          Ora fa parte del ricordo <strong style="color: #111;">"${memoryTitle}"</strong> e può aggiungere la sua versione.
        </p>
        ...
        <p style="font-size: 12px; color: #bbb; margin: 0; line-height: 1.6;">
          Hai ricevuto questa email perché sei il creatore del ricordo "${memoryTitle}" su Appnd.<br>
